...
```
[1권 결과물]                        [2권 결과물]
정적 HTML 페이지            →       Vue.js + TypeScript SPA
수동 업데이트               →       Cloudflare Pages 자동 배포
없음                        →       JWT 회원가입·로그인
없음                        →       Stripe 결제 연동
없음                        →       vue-i18n 다국어(한·영·러)
브라우저 웹사이트             →       iOS·Android 앱스토어 출시
```
...
- <mark style="color:$success;">\[혜택 1]</mark> 해당 **책 내용 전체를 하나로 묶은 PDF 파일**을 드립니다. (_후원완료 후, **후원하신 날짜와 시각, 입금자명을 적어서** 위에 언급된 저저의 이메일로 메일 한 통 남겨주세요. 해당 메일의 회신으로 파일 보내드리겠습니다. 감사합니다.)_
- <mark style="color:$success;">\[혜택 2]</mark> 마스터편 출간 전 책을 가장 먼저 읽어보실 수 있게 **베타 리더**로 초대됩니다.
