...
1권(기초편)의 부록에서 우리는 '로컬 개발 환경'의 필요성에 대해 짧게 맛보았습니다. 온라인 AI 스튜디오(Stitch 등)는 클릭 한 번에 눈앞에 결과물을 띄워주는 마법 같은 도구였지만, **진짜 상용 서비스를 만들기에는 다음과 같은 뚜렷한 한계**가 존재했습니다.

- **프로젝트 규모의 한계**: 파일이 수십 개로 늘어나고 폴더 구조가 복잡해지면, 하나의 웹 화면 안에서 모든 것을 관리하는 온라인 AI 스튜디오는 이를 감당하기 어렵습니다.
...
{% hint style="success" %}
**온라인 AI 스튜디오는 '초고속 3D 프린터', 로컬 AI기반 IDE는 '정밀 제조 공장'** 아이디어를 눈앞에 즉시 출력해 검증하는 데는 온라인 스튜디오만 한 혁신이 없습니다. 하지만 실제 도로를 달릴 양산형 자동차를 만들려면 엔진을 튜닝하고 안전장치를 결합하는 독립된 제조 공장(로컬 환경)으로 넘어가야 합니다.
{% endhint %}
...
- ✅ **1권과 연결되는 '구글 스마트 파이프라인'** : 타사 툴을 쓰면 1권에서 작업한 AI Studio 생성한 코드들에 대해 다시 재학습하는 과정이 필요합니다. 하지만 Antigravity IDE는 같은 구글 코어 엔진을 공유하므로 **AI Studio 파일과 가이드라인을 변환 과정 없이 그대로 이어받습니다.** 프로젝트를 여는 순간, 내장 AI가 우리가 1권에서 의도한 디자인 구조와 개발 방향을 백그라운드에서 이미 완벽하게 인지한 상태로 시작합니다.
- **✅ 지갑이 가벼운 개발자를 위한 압도적인 가성비 (무료 토큰 혜택)**: 타사 AI IDE들은 한 달에 몇십 번만 진지하게 코딩해도 금방 무료 쿼리가 끝나고, 결국 매달 최소한 20달러 이상의 비싼 구독료를 요구합니다. 반면 Antigravity IDE는 구글 생태계의 인프라를 등에 업고 **비교가 안 될 정도로 여유롭고 풍부한 무료 토큰(API 쿼리 포함)을 기본 제공**합니다. 결제창 앞에서 머뭇거릴 필요 없이, 중소형 규모의 상용 서비스를 완성할 때까지 비용 부담 없이 대규모 코드를 AI와 협업으로 개발할 수 있습니다.
...
<figure><img src=".gitbook/assets/antigravity_interface.png" alt="Antigravity IDE 화면 구성 설명"><figcaption><p>그림 1-1. Antigravity IDE 메인 인터페이스 영역 구성</p></figcaption></figure>
...
{% hint style="warning" %}
**⚠️ AI 모델 선택 가이드: Gemini Flash vs Gemini Pro**

Antigravity IDE 하단/우측의 채팅창 하단 모델 선택 메뉴에서는 **Gemini Flash**와 **Gemini Pro** 등 다양한 AI 모델을 선택할 수 있습니다. 복잡한 추론(Reasoning) 과정을 거치는 고지능 모델일수록 복잡한 문제 해결력이 뛰어나지만, **무료 사용자의 일일 한도가 훨씬 빠르게 소진됩니다.**

**작업 성격에 맞춰 AI 모델을 스마트하게 교체하세요:**

- **기본 작업 (Gemini Flash 권장)**: 파일 생성, 간단한 UI 스타일 수정, 반복적인 HTML/CSS 작성, 단답형 코드 설명 요청 등 일반적인 작업에는 빠르고 한도 차감이 적은 **Flash (High) 모델**로도 충분합니다.
- **고난도 작업 (Gemini Pro 모델 추천 시점)**:
...
**이렇게 시작하는 것을 권장합니다:** 평소에는 **Flash 모델**로 빠르게 대화하며 작업하고, AI가 해매거나 복잡한 아키텍처/심각한 에러를 만났을 때만 **Reasoning (High-Intelligence) 모델인 Gemini Pro** 스위칭하세요. 한도를 효율적으로 관리하면서도 필요할 때 최고의 AI 성능을 끌어낼 수 있습니다.

**💡 무료토큰 사용량 소진시 팁: 구글 계정을 2\~3개 더 만드세요.**

만약, 작업 중 개별 구글 계정마다 부여된 무료토큰이 다 소진되어 진행 중인 작업이 중간에 중단될 수 있습니다. 이 때, 다시 작업을 이어나가기 위해선 당연히 무료토근의 복원이 필요한데, 최소 4시간에서 며칠을 기다려야 할 수 있습니다. 물론 경제적으로 여유가 있다면, 이 타이밍이 유독구독을 시작할 타이밍이긴 합니다. 하지만, 모두가 그럴 수 있는 상황은 아니고, 이 때 사용할 수 있는 팁으로 현재 IDE에 로그인 되어 있는 구글 계정에서 **로그아웃 한 후, 다른 여러분들의 여분의 계정으로 재로그인해 그 계정에 남아있는 무료토큰 할당량을 할용해 곧바로 작업을 계속이어나갈 수 있습니다.**

중단된 작업을 계속 이어나기 위한 방법은 따로 있는게 아니라, 프로프트 대화창에 그냥 "**계속 진행해**" 라고만 작업지시 하면 됩니다.
{% endhint %}
...
1. **터미널(Terminal) 열기 및 설치 확인**:
   - **터미널(Terminal)이란?**: 평소 마우스로 아이콘이나 폴더를 더블 클릭해 프로그램을 실행하듯, 터미널은 **글자(명령어)를 직접 타이핑하여 컴퓨터에게 일을 시키는 대화 창구**입니다. 해커 영화의 검은 창처럼 생겨 처음엔 낯설 수 있지만, 로컬 개발 환경에서 컴퓨터의 파일과 프로그램을 다루는 가장 기본적이고 핵심적인 도구입니다.
   - **터미널 여는 법 (2가지)**:
     - **메뉴로 열기**: Antigravity IDE 상단 메뉴 표시줄에서 `Terminal` → `New Terminal`을 클릭합니다.
     - **단축키로 열기**: 키보드의 `Ctrl + (Mac:`Cmd + )를 누릅니다. (키보드 맨 윗줄 숫자 `1` 키 바로 왼쪽에 있는 물결 `~` 밑의 백틱(\`) 키입니다.)
   - **명령어 입력 및 실행**:
     - 터미널을 열면 화면 하단에 검은 패널이 나타나고, 깜빡이는 커서(`_` 또는 `|`)가 보입니다.
     - 여기에 `git -v`라고 정확히 타이핑한 뒤 **`Enter` (엔터)** 키를 누릅니다.
     - `git version 2.x.x`처럼 버전 번호가 찍혀 나오면 이미 내 컴퓨터에 Git이 잘 설치되어 있는 것입니다. 만약 `command not found` 같은 메시지가 나온다면 아래 2번 단계로 넘어가 설치를 진행합니다.
2. **다운로드 및 설치**:
   - **Windows**: [Git 공식 홈페이지(git-scm.com)](https://git-scm.com/download/win)에서 64-bit 버전을 다운로드하고, 기본 설정(계속 'Next' 클릭)으로 설치를 완료합니다. (설치 중 **"Adjusting your PATH environment"** 단계에서 **"Git from the command line and also from 3rd party software"** 옵션이 선택되어야 Antigravity IDE가 Git을 인식할 수 있습니다.)
...
4. **최초 1회 사용자 등록**: Git을 처음 설치했다면 이름과 이메일을 등록해야 합니다. 방금 열어둔 터미널 창의 깜빡이는 커서에 아래 2줄을 한 줄씩 복사해 붙여넣고(또는 타이핑하고) 각각 `Enter`를 누르세요. (GitHub 가입 이메일 사용 권장)
...
{% hint style="info" %}
이 설정을 건너뛰면 나중에 커밋할 때 "Author identity unknown" 에러가 납니다.
{% endhint %}
{% endstep %}
...
1. Antigravity IDE로 돌아와 상단 메뉴에서 `View` → `Command Palette` (`Ctrl+Shift+P` / `Cmd+Shift+P`)를 열고, `Git: Clone`을 검색해 선택합니다.
2. **1권에서 만든 소스코드 원본이 저장되어 있는 GitHub 레포지토리**의 URL(`https://github.com/내아이디/samarkand-local-mate.git`)을 붙여 넣고 Enter를 누릅니다.
3. 내 컴퓨터에서 **코드를 저장할 폴더**(예: Documents 또는 바탕화면 등)를 선택하면, 해당 폴더에 프로젝트가 복제됩니다.
4. 완료 후 우측 하단 팝업에서 **`Open`** 버튼을 클릭하면 IDE가 해당 폴더를 자동으로 엽니다.
...
환경 점검이 완료됐으면, 본격적인 마이그레이션에 앞서 **현재 IDE에 열려있는 원본 바닐라(Vanilla) JS 프로젝트의 구조**를 다시한번 파악해 봅시다. AI에게 과업을 시키기 전에, '지금 내가 다루려는 재료가 무엇인지'를 이해하는 과정입니다.

좌측 탐색기(Explorer)에서 파일들을 살펴보면 대략 이런 구조일 것입니다:

```
프로젝트 폴더 (예: samarkand-local-mate)
├── index.html        ← 앱의 프런트엔ㅌ드 전체를 담고 있는 단 하나의 파일
└── functions/
    └── api/          ← 백엔드 API 코드 (Cloudflare Workers)
```

이것이 1권 방식의 전형적인 구조입니다. 파일 수는 적지만 `index.html` 하나에 모든 것이 담겨 있어, 기능이 늘수록 관리가 어려워집니다. 바로 이 구조를 2장에서 Vue 컴포넌트 방식으로 분해하고 재조립하게 됩니다.

{% hint style="success" %}
**🎯 이 장의 목표 달성!** 원본 코드가 IDE에 열려 있고, Node.js 환경 점검도 완료됐습니다. 다음 장에서는 곧바로 이 원본 코드를 Vue.js로 변환하는 실전 마이그레이션을 시작합니다!
{% endhint %}

## 5. 미리 보기: 마이그레이션 후 등장할 'Vite'와 핫 리로드(HMR)

1권에서는 HTML 파일을 고치고 저장한 뒤 브라우저를 직접 새로고침(F5)해야 변경 사항을 확인할 수 있었습니다. 2장에서 마이그레이션을 마치고 나면 완전히 다른 경험을 하게 됩니다.

**Vite(비트)**&#xB294; Vue.js 생태계의 빌드 도구로, 우리가 작성한 최신 코드를 브라우저가 이해할 수 있는 HTML/CSS/JS로 순식간에 번역해 줍니다. 그리고 그 결정적인 장점이 바로 **HMR(Hot Module Replacement)** 기능입니다.

2장에서 마이그레이션이 끝난 뒤 터미널에서 `npm run dev` 명령어로 로컬 서버를 띄우고, 파일을 수정해서 저장해 보세요. 브라우저가 새로고침 없이 그 즉시 변경 사항을 반영합니다. AI가 코드를 고치는 동시에 브라우저에서 결과를 실시간으로 확인할 수 있어, 바이브 코딩의 '즉각적인 피드백 루프'가 완성됩니다.

{% hint style="info" %}
**💡 지금은 아직 `npm run dev`를 온전히 실행 시킬 수 없습니다.** 현재 열려있는 원본 Vanilla JS 프로젝트는 Vite가 설치되어 있지 않아 이 명령어가 없습니다. 다음 장에서 Vue + Vite 마이그레이션을 마친 뒤에야 온전히 이 기능을 체험할 수 있습니다.
{% endhint %}
