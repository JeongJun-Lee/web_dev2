...
## 0. 시작하기 전에: 든든한 깃(Git) 안전망이 이미 준비되어 있습니다

1장에서 GitHub Clone을 통해 코드를 가져왔기 때문에, 현재 폴더는 **1권의 최종 완성본 상태가 이미 온전히 커밋(기록)되어 있는 상태**입니다.

따라서 지금 별도로 백업을 하거나 초기 커밋을 남기는 번거로운 작업을 할 필요가 전혀 없습니다! 아무것도 건드리지 않은 깨끗한 원본 상태가 Git 히스토리에 든든한 '복구 지점'으로 이미 박혀 있으므로, 앞으로 AI가 마이그레이션 도중 코드를 엉키게 만들더라도 언제든 1권의 마지막 원본 시점으로 깨끗하게 되돌릴 수 있습니다.

안전망이 확보되었으니 안심하고 본격적인 Vue 변환을 AI에게 맡겨보겠습니다.

{% hint style="tip" %}
**💡 앞으로 작업이 끝날 때마다 커밋을 AI에게 위임하세요**

이미 우리는 1권에서부터 커밋하는 습관을 익혀와서 생소한 것은 아니고, 이번 장에서도 목표인 **Vue 마이그레이션이 성공적으로 끝나면**, 그때도 AI에게 **"지금까지 작업한 Vue 변환 내용을 요약해서 커밋해줘"** 라고 지시해 새로운 복구 지점을 만들면 됩니다. 앞으로도 매 챕터마다 하나의 작업을 끝내고 나면 AI에게 커밋을 시키는 습관을 들이면 안전하게 개발할 수 있습니다.
{% endhint %}
...
2. **계획 검토 및 승인**: 화면에 제시된 계획을 아직은 전체적으로 다 이해할 수 없겠지만, 그럼에도 불구하고 늘 한번 읽어보고 점검하는 습관을 들이는 것이 중요하기 때문에 내용을 이해할 수 있는 선까지 확인한 뒤, AI 채팅창 하단이나 상단 팝업에 있는 **`[Proceed]` (진행하기)** 버튼을 클릭합니다.
...
4. **자율 화면 점검 & 도구 실행 권한 승인 (`Approve`)**:
   ...
   - AI가 주어진 과업을 수행 중 IDE 하단이나 채팅창에 **터미널 명령어 실행 또는 브라우저 실행을 허용할지 선택하는 팝업**(`Approve` / `Allow`)이 뜹니다.
   - 이때 **`[Approve]` (승인)** 버튼을 클릭해 주면 AI가 직접 브라우저를 컨트롤하며 기능 검증까지 마치게 됩니다.
