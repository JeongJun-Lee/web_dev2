...
**✅ 1권 원본 코드를 현재 워크스페이스(Workspace)에 복사하여 제공하기 (추천)**

Antigravity IDE는 **현재 열려 있는 워크스페이스(프로젝트 폴더) 내의 파일만** AI 채팅창으로 `@` 호출하거나 드래그 앤 드롭할 수 있습니다. 따라서 아래 순서대로 진행하는 것이 가장 쉽습니다.

1. 1권에서 만들었던 원본 소스 코드 폴더를 현재 작업 중인 `s-local-mate-vue` 폴더 안에 `legacy-code`라는 이름의 하위 폴더로 복사해 넣습니다.
2. IDE 좌측 탐색기(Explorer)에 `legacy-code` 폴더가 나타나면, 이 폴더나 파일을 우측 AI 채팅창으로 **드래그 앤 드롭**하거나 입력창에서 **`@legacy-code`**를 입력해 첨부합니다.
3. 변환 작업이 모두 완료되면 `legacy-code` 폴더는 삭제하거나 참고용으로 남겨둡니다.

{% hint style="warning" %}
**⚠️ `@` 기호에 URL을 직접 입력하면 안 됩니다!**
`@` 메뉴는 **현재 프로젝트(워크스페이스) 내부의 파일과 폴더**를 검색하는 기능입니다. `@https://github.com/...`처럼 외부 URL을 `@` 뒤에 직접 붙여 치면 `No matching results` 에러가 발생합니다. 외부 GitHub 저장소 링크를 전달하고 싶다면 `@` 없이 **프롬프트 텍스트에 URL을 그대로 붙여넣기** 하시면 됩니다.
{% endhint %}

전체 코드 맥락이 AI에게 제공되었다면, 아래와 같이 프롬프트를 입력합니다.

> 💬 "제공한 `legacy-code` 폴더(또는 GitHub 저장소 링크)가 기존 사마르칸트 투어 앱의 원본 코드야. 이걸 기반으로 현재 세팅된 Vue.js 프로젝트에 프론트엔드 코드를 변환해서 덮어씌워줘. 기존의 `functions/api` 백엔드 코드는 폴더 구조 그대로 유지해서 가져와주고, 1권에서 쓰던 Tailwind CSS 스타일이 그대로 적용되도록 세팅해. 프론트엔드 UI는 헤더, 메인 콘텐츠, 푸터를 각각 별도의 Vue 컴포넌트로 분리해서 App.vue에서 조립해줘."

### 📋 프롬프트 제출 후 진행 과정: Implementation Plan, `[Proceed]`, 그리고 권한 승인

프롬프트를 전송하면 AI가 코드 작성부터 화면 검증까지의 과정을 단계별로 진행합니다.

1. **Implementation Plan (구현 계획서) 생성**: AI가 기존 코드 분석 후, "어떤 파일들을 생성/수정할 것인지" 정리한 청사진(Implementation Plan)을 먼저 만듭니다.
2. **계획 검토 및 승인**: 화면에 제시된 계획을 확인한 뒤, AI 채팅창 하단이나 상단 팝업에 있는 **`[Proceed]` (진행하기)** 버튼을 클릭합니다.
3. **실제 코드 생성 및 파일 변환**: `[Proceed]` 버튼을 누르면 비로소 AI가 파일들을 하나씩 생성하고 코드를 변환해 적용하기 시작합니다.
4. **자율 화면 점검 & 도구 실행 권한 승인 (`Approve`)**:
   - 고성능 AI 모델은 파일 수정 후 끝내는 것이 아니라, 개발 서버(`npm run dev`)를 직접 켜고 브라우저 도구를 실행해 화면이 깨지지 않는지, 버튼이나 애니메이션이 정상 동작하는지 직접 테스트합니다.
   - 이때 IDE 하단이나 채팅창에 **터미널 명령어 실행 또는 브라우저 실행 허가 팝업**(`Approve` / `Allow`)이 뜹니다.
   - **`[Approve]` (승인)** 버튼을 클릭해 주면 AI가 직접 브라우저를 컨트롤하며 기능 검증까지 마치게 됩니다.
