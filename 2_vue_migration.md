...
{% hint style="warning" %}
그러나, AI가 생성한 결과, 즉, 여러분들 컴퓨터에 있는 파일의 이름과 내용이 항상 책 내용과 100% 같지 않을 수 있음을 늘 유념해야 합니다. 그것은 AI가 기본적으로 갖는 특성인데 동일한 프롬프트에 대해 100% 동일한 결과를 보장하지 않기 때문입니다.
{% endhint %}

기존의 html 한 개로 완성체였던 우리의 앱이 이제는 여러 개의 vue 파일로 쪼개졌습니다. 이 vue 파일 하나하나가 콤포넌트(component)라고 불리며, 완성된 전체 작품(또는 프로그램)을 구성하는 일종에 레고의 개별 블록 부품이라고 생각하면 이해하기 쉽습니다. App.vue 파일은 이러한 부품들을 조립하는 설명서 같은 역할을 합니다. 이렇게 독립된 부품으로 나눠서 개발하는 것을 앞선 1장에서 다뤘듯이 **컴포넌트 기반 개발(CBD: Component-Based Development)** 방법론 이라고 부르고, 이러한 개발 방식의 장점은 레고 블록은 단회성이 아니라 필요에 따라 얼마든지 동일한 부품을 반복적으로 재사용하여 또 다른 작품의 구성품이 될 수 있듯이 우리 역시 하나의 콤포넌트를 잘 개발해 두면 필요에 따라 얼마든지 복제하여 재사용하거나 다른 컴포넌트에 결합하여 더 크고 복잡한 프로그램을 구성할 수 있다는 점입니다. 이를 코딩영역에서는 **코드의 재사용성(Reusability)** 이라고 부릅니다. 또한 각 콤포넌트가 독립된 파일로 존재하기 때문에 여러 개발자가 각자 맡은 콤포넌트를 나눠서 동시에 개발하는 데 용이하다는 점도 장점입니다.

그밖에 CBD의 또 다른 장점은 유지보수의 용이성이 향상된다는 점입니다. 예를 들어 레고 블록으로 만든 성에서 창문 부분이 조금 마음에 들지 않을 경우 전체를 분해해서 다시 조립할 필요없이 마음에 들지 않는 창문 부품만 교체하면 되는 것과 같이, 유지보수 시 문제가 있는 특정 파일만 수정하면 되기 때문입니다. 또 다른 유지보수의 측면으로 만약 프로그램의 작동을 중지시키는 에러가 발생할 때, 그 에러를 부품이라는 지역 안에 가두어 전체로 퍼지지 않고 원인이 있는 해당 파일 안에서만 고립되기 때문입니다. 그래서 유지보수가 훨씬 용이해집니다. 프로그램 개발에서는 이를 **지역성(Locality)**&#xC774;라고 표현합니다.

{% hint style="info" %}
**💡 앞으로 작업이 끝날 때마다 커밋을 AI에게 위임하세요**

이미 우리는 1권에서부터 매 작업단위마다 커밋하는 습관을 익혀와서 생소한 것은 아닙니다. 이번 장에서도 목표인 **Vue 마이그레이션이 성공적으로 끝나면**, 그때도 AI에게 **"지금까지 작업한 내용을 요약해서 커밋해줘"** 라고 지시해 매번 커밋을 통해 새로운 복구 지점을 만들면 됩니다. 이렇게 앞으로도 하나의 작업을 끝내고 나면 AI에게 커밋을 시키는 습관을 들이면 안전하게 개발할 수 있습니다.
{% endhint %}
...
IDE의 **Extension(확장 프로그램)**&#xC740; 마치 처음에 아무것도 없는 깨끗한 스마트폰을 사서 필요한 게임이나 번역 앱을 필요에 따라 **'앱 스토어에서 추가로 앱을 설치하는 것'**&#xACFC; 같습니다. 최초 설치된 개발 환경(IDE) 내 에 특정 언어나 프레임워크(Vue, Python 등)를 더 똑똑하게 지원해 주는 보조 도구를 추가하는 것입니다.
{% endhint %}
...
1권에서 CSS를 전역 파일 하나로 관리하다 보면, `index.html` 어딘가에서 `.title { color: red; }`를 선언했는데 전혀 엉뚱한 곳의 제목까지 빨개지는 충돌 사고가 생기곤 했습니다. `scoped`는 이 문제를 근본적으로 해결합니다. 이것은 마치 위에 CBD 개발 방법론에서의 언급된 지역성(locality) 원칙과 일맥상통합니다.
...
하지만, 우리 앱은 Tailwind CSS를 전면 사용하므로 `<style scoped>` 영역이 거의 비어 있습니다. 특별히 Tailwind로 표현하기 어려운 복잡한 애니메이션 같은 경우에만 여기에 작성합니다.
...
// Vue — ref 변수: 값이 바뀌는 순간 Vue 프레임워크가 이를 감지하고 화면을 자동으로 업데이트
import { ref } from "vue";
const mobileMenuOpen = ref(false);
mobileMenuOpen.value = true; // 화면의 모바일 메뉴가 즉시 나타남!
```

참고로 `ref` 변수의 실제 값을 읽거나 쓸 때는 JS 코드 안에서 `.value`를 붙여야 합니다. 하지만 `<template>` HTML 구역 안에서는 Vue가 자동으로 `.value`를 처리해주므로 `{{ mobileMenuOpen }}`처럼 그냥 쓰면 됩니다.
...
const emit = defineEmits<{
  (e: "close"): void; // 닫기 이벤트
  (e: "submit", data: FormData): void; // 가이드 매칭 요청 폼 제출 이벤트 (데이터와 함께)
}>();
</script>
...
<template>
  ...
  <!-- 가이드 매칭 요청 폼 제출 시 → 부모에게 'submit' 이벤트와 함께 사용자 입력 데이터를 전달 -->
...
또한 `v-model="tourType"`이라는 표현도 보입니다. 이것은 폼 입력(`<select>`, `<input>`)과 데이터를 **양방향으로 자동 연결**해주는 마법입니다. 사용자가 선호하는 관광타입을 고르는 셀렉트 박스에서 다른 옵션을 고르면 `tourType.value`가 자동으로 바뀌고, 반대로 코드에서 `tourType.value = 'food'`로 바꾸면 셀렉트 박스의 선택 항목이 자동으로 "음식 투어"로 바뀝니다.
...
## 7. 마치며: 2장을 정리하고 다음 단계로

이번 2장에서는 1권에서 작성했던 긴 바닐라 JavaScript 코드를 한 단계 발전시켜, 모던 웹 프레임워크인 **Vue 3** 기반의 앱으로 성공적으로 마이그레이션했습니다.

우리는 이번 장을 통해 다음과 같은 중요한 웹 개발의 전환점을 경험했습니다.

* HTML, JavaScript, CSS를 한 파일에 캡슐화해 레고 블록처럼 조립하는 **싱글 파일 컴포넌트(SFC)** 구조를 익혔습니다.
* 복잡한 DOM 조작 함수 없이도 `{{ }}`, `v-if`, `v-for`, `:bind`, `@click`, `v-model`과 같은 템플릿 문법만으로 화면을 간결하게 표현했습니다.
* `props`와 `emit`을 활용해 부모-자식 컴포넌트가 체계적으로 데이터를 주고받는 통신 방식을 배웠습니다.
* 무엇보다 `ref`와 `computed`를 통해 상태(데이터)가 바뀌면 화면이 자동으로 업데이트되는 **반응형(Reactivity) 패러다임**을 체득했습니다.

이제 우리 앱은 기능별로 조각나 조립하기 쉬운 탄탄한 컴포넌트 구조를 갖추었습니다. 하지만 여전히 한 가지 위험이 남아 있습니다. 가이드 데이터의 평점(`rating`)에 실수로 문자열을 넣거나 필수 데이터가 누락되더라도, 사용자가 앱을 실행해 화면이 하얗게 멈추기 전까지는 문제를 알아채기 어렵다는 점입니다.

이어지는 **제3장: TypeScript — 코드에 안전망 씌우기**에서는 데이터의 정확한 모양을 미리 약속하는 설계도인 \*\*인터페이스(Interface)\*\*와 기본 타입 시스템을 배웁니다. 실수를 저지르는 순간 에디터가 빨간 밑줄로 콕 집어 알려주고, AI가 찰떡같이 정확한 코드를 작성할 수 있도록 돕는 TypeScript의 강력한 안전망을 함께 씌워보겠습니다.
